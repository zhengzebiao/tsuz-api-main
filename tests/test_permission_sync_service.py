...
def test_first_sync_creates_real_catalog_bindings_and_admin_grants_idempotently(
    ...
    assert len(plan.created) == 21
    assert len(plan.endpoint_bindings_added) == 26
    assert len(plan.admin_grants_added) == 21
    ...
    assert summary.to_dict() == {
        ...
        "restored": 0,
        "marked_missing": 0,
        "endpoint_bindings_added": 26,
        "endpoint_bindings_removed": 0,
        "admin_grants_added": 21,
        "sessions_revoked": 2,
        "unchanged": 0,
    }
    assert db_session.scalar(select(func.count()).select_from(Permission)) == 21
    assert db_session.scalar(select(func.count()).select_from(PermissionEndpoint)) == 26
    assert db_session.scalar(select(func.count()).select_from(role_permissions)) == 21
