...
def test_first_sync_creates_real_catalog_bindings_and_admin_grants_idempotently(
    ...
    assert len(plan.created) == 33
    assert len(plan.endpoint_bindings_added) == 40
    assert len(plan.admin_grants_added) == 33
    ...
    assert summary.to_dict() == {
        ...
        "restored": 0,
        "marked_missing": 0,
        "endpoint_bindings_added": 40,
        "endpoint_bindings_removed": 0,
        "admin_grants_added": 33,
        "sessions_revoked": 2,
        "unchanged": 0,
    }
    assert db_session.scalar(select(func.count()).select_from(Permission)) == 33
    assert db_session.scalar(select(func.count()).select_from(PermissionEndpoint)) == 40
    assert db_session.scalar(select(func.count()).select_from(role_permissions)) == 33
